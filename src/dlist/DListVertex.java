...
public class DListVertex implements IList {
	...
	public Integer getVertexAt(int index) {
		int i = 0;
		Integer result=null;
		for (DNodeVertex nodeIt = header.next; nodeIt != trailer && result==null; nodeIt = nodeIt.next) {
			if (i == index) {
	...
	public Float getWeightAt(int index) {
		...
		for (DNodeVertex nodeIt = header.next; nodeIt != trailer && result==null; nodeIt = nodeIt.next) {
			if (i == index) {
				result=nodeIt.weight;
			}
			++i;
		}
		if (result==0) System.out.println("DList: Get out of bounds");
		return result;
	}
